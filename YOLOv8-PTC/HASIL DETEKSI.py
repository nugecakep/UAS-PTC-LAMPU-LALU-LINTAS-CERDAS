...
count = 0
car_count = 0
total_car_detected = 0  # Counter for total cars detected across all frames
bus_count = 0
truck_count = 0
tracker = Tracker()
offset = 8
...
while True:
    ...
    if elapsed_time > 30:  # Stop processing after 30 seconds
        break
    ...
    for bbox in cars_boxes:
        ...
        if (cy > 184 - offset) and (cy < 184 + offset):
            car_count += 1
        total_car_detected += 1  # Increment total cars detected
...
print(f'Total cars detected across all frames: {total_car_detected}')
